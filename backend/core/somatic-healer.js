...
export class SomaticHealer {
  ...
  async generateSession(data) {
    try {
      ...
      const trustIntegration = await this.integrateTrustBuilding(
        analysis,
        session,
      );
      ...
      try {
        ...
        logResult = await writeLog(this.env, {
          type: "somatic_session",
          payload: sessionData,
          session_id: data.session_id || null,
          who: data.who || null,
          level: analysis.energy_level || "moderate",
          tags: data.tags || [],
          textOrVector: data.body_state || null,
        });
      } catch (e) {
        logResult = { error: String(e) };
      }
  ...
  assessReadinessForBodyWork(bodyState, emotions) {
    ...
    if (
      lowerBodyText.includes("ready") ||
      lowerEmotions.includes("curious") ||
      lowerEmotions.includes("open")
    )
      return "high";
    if (
      lowerBodyText.includes("resist") ||
      lowerEmotions.includes("overwhelmed") ||
      lowerEmotions.includes("scared")
    )
      return "low";
    return "moderate";
  }
  ...
  generateInitialBodyInsights(bodyState, emotions) {
    ...
    if (bodyState.includes("tension") || bodyState.includes("tight")) {
      insights.push(
        "Tension is your body's way of protecting you - let's work with it compassionately rather than forcing release",
      );
    }

    if (emotions.includes("anxious") && bodyState.includes("chest")) {
      insights.push(
        "Anxiety in the chest often indicates your heart wanting protection while staying open to life",
      );
    }

    if (bodyState.includes("heavy")) {
      insights.push(
        "Heaviness in the body can be a sign of unexpressed emotions or energy that needs movement",
      );
    }

    if (bodyState.includes("disconnected") || bodyState.includes("numb")) {
      insights.push(
        "Disconnection is a protective strategy - your body will reconnect when it feels safe to do so",
      );
    }
    ...
    return insights.length > 0
      ? insights
      : [
          "Your body is always communicating - let's listen together with curiosity and compassion",
        ];
  }
  ...
  generateSessionPhases(analysis) {
    ...
    phases.push({
      ...
      purpose:
        "Coming into presence with your body and creating internal safety",
    ...
    phases.push({
      ...
      purpose:
        "Developing relationship with your body's communication and wisdom",
    ...
    phases.push({
      ...
      purpose:
        "Integrating the session and building trust with your body's guidance",
  ...
  getArrivalInstructions(analysis) {
    ...
    if (analysis.nervous_system_state === "activated") {
      baseInstructions.push(
        "Notice that you're activated and that this is okay - you're not trying to calm down, just acknowledging",
      );
    } else if (analysis.nervous_system_state === "hypoactivated") {
      baseInstructions.push(
        "If you feel disconnected, that's okay too - simply notice what's available to feel right now",
      );
    }
  ...
  getBodyDialogueInstructions(analysis) {
    ...
    if (analysis.tension_patterns.includes("upper_body")) {
      instructions.push(
        "Pay special attention to your shoulders and neck - what are they holding for you?",
      );
    }

    if (analysis.tension_patterns.includes("core")) {
      instructions.push(
        "Notice your chest and belly - how does your core feel right now?",
      );
    }

    instructions.push(
      "Remember: your body speaks in sensations, not words - trust whatever comes",
    );
  ...
  async integrateTrustBuilding(analysis, session) {
    return {
      ...
      standing_tall_connections:
        this.generateStandingTallBodyConnections(analysis),
    };
  }
  ...
  generateStandingTallBodyConnections(analysis) {
    ...
    if (analysis.tension_patterns.includes("upper_body")) {
      connections.push(
        "Upper body tension often reflects holding yourself up alone - true standing tall comes from internal support and groundedness",
      );
    }

    if (analysis.energy_level === "low") {
      connections.push(
        "Honor your body's energy levels - standing tall sometimes means resting so you can show up authentically",
      );
    }

    connections.push(
      "Every time you listen to your body with respect, you're practicing the self-trust that allows authentic presence",
    );
  ...
  connectToStandingTall(analysis) {
    let connection =
      "Your relationship with your body directly affects your ability to stand tall in the world. ";

    if (analysis.tension_patterns.includes("upper_body")) {
      connection +=
        "Upper body tension often reflects the effort of holding yourself up alone. True standing tall comes from feeling supported from within your own body. ";
    }

    if (analysis.nervous_system_state === "regulated") {
      connection +=
        "A regulated nervous system is the foundation for authentic presence and confident expression. ";
    }

    connection +=
      "Every moment you spend listening to your body with curiosity and respect builds the internal trust that allows you to stand in your power.";
  ...
  generateDailyPractices(analysis) {
    ...
    practices.push({
      ...
      instruction:
        'Before getting out of bed, ask your body: "How are you today? What do you need?"',
    ...
    practices.push({
      ...
      instruction:
        "Before important choices, pause and notice: does this option create expansion or contraction in your body?",
    ...
    if (analysis.tension_patterns.length > 0) {
      practices.push({
        ...
        instruction:
          "Thank your areas of tension for protecting you, then ask what they need",
        timing: "When you notice tension",
        purpose:
          "Build a collaborative relationship with your body's protective mechanisms",
      });
    }
    ...
    if (analysis.nervous_system_state === "activated") {
      practices.push({
        ...
        instruction:
          'When activated, place a hand on your heart and say "I see you, I\'m here with you"',
  ...
  generateWelcomeMessage(analysis) {
    if (
      analysis.readiness_for_work === "high" &&
      analysis.nervous_system_state === "regulated"
    ) {
  ...
  getEmergencySomaticResponse(data) {
    return {
      message:
        "Your body\'s wisdom is always available, even when systems are offline. The most important thing is your willingness to listen.",
      body_analysis: {
        message:
          "Your body is constantly communicating - simply noticing this is valuable body awareness",
      },
      somatic_session: {
      ...
      trust_integration: {
        body_trust_insights: [
          "Your body is always telling you the truth about your current state",
        ],
        trust_building_practices: [
          "Trust whatever your body shares - it\'s always valid information",
        ],
      },
      daily_body_practices: [
        {
          ...
          instruction:
            "Ask your body one question each day and listen with curiosity for the response",
          timing: "Daily",
        },
      ],
    };
