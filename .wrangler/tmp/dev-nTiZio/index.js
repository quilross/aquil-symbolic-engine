...
// src/ark/core.js
function getPhiladelphiaTime(date = /* @__PURE__ */ new Date()) {
...
__name(getPhiladelphiaTime, "getPhiladelphiaTime");
function generateId() {
  return `ark_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
}
__name(generateId, "generateId");
async function logMetamorphicEvent(env2, event) {
  try {
...
__name(logMetamorphicEvent, "logMetamorphicEvent");

// src/index.js
async function enhanceWithArk(originalResponse, context, env2) {
  try {
    ...
    await logMetamorphicEvent(env2, {
...
__name(enhanceWithArk, "enhanceWithArk");
var router = e();
var corsHeaders = {
...
async function logEvent(env2, event) {
...
async function getEvents(env2, opts = {}) {
  const {
    limit,
    type,
    who,
  ...
  `;
  params.push(lim);
  const { results } = await env2.AQUIL_DB.prepare(sql).bind(...params).all();
  return results || [];
}
...
router.post("/api/log", async (request, env2) => {
  try {
    const body = await request.json();
    const id = await logEvent(env2, body);
...
router.get("/api/logs", async (request, env2) => {
  try {
    const url = new URL(request.url);
    const opts = Object.fromEntries(url.searchParams.entries());
    const events = await getEvents(env2, opts);
...
router.get("/api/session-init", async (request, env2) => {
  try {
    const url = new URL(request.url);
...
var src_default = {
  async fetch(request, env2, ctx) {
    return router.handle(request, env2, ctx).catch((err) => {
...
// .wrangler/tmp/bundle-Wvv6zX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Wvv6zX/middleware-loader.entry.ts
