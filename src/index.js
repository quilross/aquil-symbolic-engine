...
// UUID v4 regex and helpers
const UUID_V4 = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/
const LOG_TYPES = new Set(['session','voice-change','insight','breakthrough','commitment','api-failure','session-end'])
const STORED_IN = new Set(['KV','D1'])
const MAX_DETAIL = actions.guards?.maxDetailLength ?? 4000

const Routes = actions.routes

// Ensure D1 schema exists (runs fast; CREATE IF NOT EXISTS is cheap)
...
// Minimal validation without deps
function isIso(ts) {
  // let Date parse and ensure round-trip validity
  try {
    const d = new Date(ts)
    return !isNaN(d.getTime()) && d.toISOString().startsWith(ts.slice(0, 19))
  } catch { return false }
}
function validateLog(payload) {
  ...
  if (!STORED_IN.has(storedIn)) return `storedIn must be KV or D1`
  return null
}
...
async function handleRetrieve(env, req) {
  ...
  // D1 first; hybrid KV merge optional (kept simple here)
  const clauses = []
  const params = []
  ...
  // Optional hybrid: include recent KV if source !== 'D1'
  if (source !== 'D1') {
    // naive KV scan: if you keep KV keys prefixed by "log:", you can list() if available; otherwise skip to keep it lightweight
    // For minimalism, we skip KV listing here (Cloudflare KV listing can be rate-limited). D1 remains the source of truth.
  }
...
async function handleRetrievalMeta(env, req) {
  await ensureSchema(env)
  let ts
  try { ts = (await req.json())?.timestamp } catch { /* empty body ok */ }
...
// Wire routes from the JSON spec to handlers
router.post(Routes.kvWrite, async (req, env) => {
  try {
    return await handleKvWrite(env, req);
  } catch (error) {
...
router.post(Routes.d1Insert, async (req, env) => {
  try {
    return await handleD1Insert(env, req);
  } catch (error) {
...
router.post(Routes.promote, async (req, env) => {
  try {
    return await handlePromote(env, req);
  } catch (error) {
...
router.post(Routes.retrieve, async (req, env) => {
  try {
    return await handleRetrieve(env, req);
  } catch (error) {
...
router.get(Routes.retrieveLatest, async (req, env) => {
  try {
    return await handleRetrieveLatest(env, req);
  } catch (error) {
    console.error('Retrieve latest error:', error);
    return json({ ok: false, error: error.message }, { status: 500 });
  }
});
...
router.post(Routes.retrievalMeta, async (req, env) => {
  try {
    return await handleRetrievalMeta(env, req);
  } catch (error) {
    console.error('Retrieval meta error:', error);
    return json({ ok: false, error: error.message }, { status: 500 });
  }
});
...
export default {
  async fetch(req, env, ctx) {
    ...
    // Wire routes from the JSON spec to handlers (new logging functionality)
    if (req.method === 'POST' && pathname === Routes.kvWrite)     return handleKvWrite(env, req)
    if (req.method === 'POST' && pathname === Routes.d1Insert)    return handleD1Insert(env, req)
    if (req.method === 'POST' && pathname === Routes.promote)     return handlePromote(env, req)
    if (req.method === 'POST' && pathname === Routes.retrieve)    return handleRetrieve(env, req)
    if (req.method === 'GET'  && pathname === Routes.retrieveLatest) return handleRetrieveLatest(env, req)
    if (req.method === 'POST' && pathname === Routes.retrievalMeta)  return handleRetrievalMeta(env, req)

    // Fallback to existing router for all other routes
    try {
