...
// =============================================================================
// LOGGING API ENDPOINTS (Alternative Path for ChatGPT Actions)
// =============================================================================

// Duplicate logging routes with /logging/api/ prefix to avoid domain conflicts
router.get("/logging/api/session-init", async (req, env) => {
  try {
...
router.post("/logging/api/log", async (req, env) => {
  try {
    const body = await req.json();
    const result = await writeLog(env, body.data, body.type || 'conversation', body.metadata || {});
    
    await logChatGPTAction(env, 'logData', body, result);
    
    return addCORS(new Response(JSON.stringify(result), {
      headers: { 'Content-Type': 'application/json' }
    }));
  } catch (error) {
    await logChatGPTAction(env, 'logData', {}, null, error);
    return addCORS(createErrorResponse({ error: error.message }, 500));
  }
});

router.get("/logging/api/logs", async (req, env) => {
  try {
    const url = new URL(req.url);
    ...
    await logChatGPTAction(env, 'retrieveLogs', { limit }, results.results);
    
    return addCORS(new Response(JSON.stringify(results.results), {
      headers: { 'Content-Type': 'application/json' }
    }));
  } catch (error) {
    await logChatGPTAction(env, 'retrieveLogs', {}, null, error);
    return addCORS(createErrorResponse({ error: error.message }, 500));
  }
});

router.post("/logging/api/logs", async (req, env) => {
  try {
    const body = await req.json();
    ...
    switch (operation) {
      case 'kv-write':
        result = await env.KV.put(payload.key, JSON.stringify(payload.value), {
          expirationTtl: payload.expirationTtl
        });
        break;
      case 'd1-insert':
        result = await env.DB.prepare(`
          INSERT INTO logs (id, type, content, metadata, timestamp)
          VALUES (?, ?, ?, ?, ?)
        `).bind(
        ...
        break;
      case 'promote':
        // Logic for promoting data between storage systems
        result = { promoted: true, operation: 'promote' };
        break;
      case 'retrieve':
        if (filters && filters.storage === 'KV') {
          result = await env.KV.get(filters.key, { type: 'json' });
        } else {
          result = await env.DB.prepare(`
            SELECT * FROM logs 
            WHERE type = ? 
            ORDER BY timestamp DESC 
            LIMIT ?
          `).bind(filters.type || 'general', filters.limit || 20).all();
        }
        break;
      case 'latest':
        result = await env.DB.prepare(`
      ...
      case 'retrieval-meta':
        result = {
          totalLogs: await env.DB.prepare(`SELECT COUNT(*) as count FROM logs`).first(),
          latestTimestamp: await env.DB.prepare(`SELECT MAX(timestamp) as latest FROM logs`).first()
        };
        break;
      default:
    ...
    await logChatGPTAction(env, 'advancedLogging', body, result);
    
    return addCORS(new Response(JSON.stringify(result), {
      headers: { 'Content-Type': 'application/json' }
    }));
  } catch (error) {
    await logChatGPTAction(env, 'advancedLogging', {}, null, error);
    return addCORS(createErrorResponse({ error: error.message }, 500));
  }
});

router.get("/logging/api/search/logs", async (req, env) => {
  try {
    const url = new URL(req.url);
    ...
    await logChatGPTAction(env, 'searchLogs', { query, limit }, results.results);
...
router.get("/logging/api/search/r2", async (req, env) => {
  try {
    const url = new URL(req.url);
    const prefix = url.searchParams.get('prefix') || '';
    const limit = parseInt(url.searchParams.get('limit')) || 20;
    
    const objects = await env.BUCKET.list({ prefix, limit });
    
    await logChatGPTAction(env, 'searchR2', { prefix, limit }, objects);
    
    return addCORS(new Response(JSON.stringify(objects), {
      headers: { 'Content-Type': 'application/json' }
    }));
  } catch (error) {
...
router.post("/logging/api/rag/search", async (req, env) => {
  try {
    const body = await req.json();
    const { query, limit = 5, namespace } = body;
    ...
    await logChatGPTAction(env, 'ragSearch', body, results);
