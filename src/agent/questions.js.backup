...
async function callDiscoveryEndpoint(env, { voice, pressLevel, text, context }) {
  try {
    // For internal calls, we need to import and call the handler directly
    const { handleDiscoveryInquiry } = await import('../ark/endpoints.js');
    ...
    const mockRequest = {
      json: async () => ({
        ...
        depth_level: Math.min(pressLevel, 3), // Map 1-4 to 1-3
        context: context.session_id || 'unknown'
      })
    };
    
    const response = await handleDiscoveryInquiry(mockRequest, env);
...
async function callSocraticEndpoint(env, { voice, text, context }) {
  try {
    ...
    // We'll return a simple socratic question based on voice
