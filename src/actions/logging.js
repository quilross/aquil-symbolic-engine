  ...
  if (env?.VECTORIZE?.upsert) {
    try {
      ...
      await writeWithRetry(() =>
        env.VECTORIZE.upsert([{ id, values, metadata: log }])
      );
    } catch {
...
async function vectorSearch(env, query) {
  ...
  const res = await env.VECTORIZE.query(q);
...
export async function advancedLoggingOperations(env, { operation } = {}) {
  ...
  for (const { name } of keys) {
    ...
    try {
      await env.VECTORIZE.upsert([{ id, values: log.vector || [0], metadata: log.metadata }]);
    } catch {
      errs.push('Vector');
    }
    if (errs.length === 0) {
