...
export class TrustBuilder {
  ...
  async analyzeTrustState(data) {
    ...
    const analysis = {
      ...
      body_awareness: body_sensations
        ? this.ai.extractBodySensations(body_sensations)
        : [],
  ...
  analyzeSituation(situation) {
    ...
    const matchedTypes = Object.entries(situationTypes)
      .filter(([type, keywords]) =>
        keywords.some((keyword) => lowerSituation.includes(keyword)),
      )
      .map(([type]) => type);
  ...
  identifyGrowthOpportunity(situationTypes) {
    const opportunities = {
      ...
      conflict:
        "Stand in your truth while remaining open to others' perspectives",
      boundary: "Honor your energy and needs as valid and important",
    };

    return situationTypes.map(
      (type) =>
        opportunities[type] || "Trust your inner knowing in this situation",
    );
  }
  ...
  async generateTrustGuidance(analysis) {
    ...
    return {
      ...
      internal_authority_guidance:
        this.generateInternalAuthorityGuidance(analysis),
    };
  }
    ...
    trustBlocks.forEach((block) => {
      switch (block) {
        ...
        case "past_disappointment":
          guidance[block] = {
            insight:
              "Past experiences inform but don't determine future outcomes. You've grown since then.",
            practice:
              "Acknowledge the past disappointment and also notice how you've evolved.",
            reframe:
              "Previous disappointments were learning experiences, not predictions of future failure.",
          };
          break;
        case "fear_of_failure":
          guidance[block] = {
            insight:
              "Fear of failure often indicates you care deeply about the outcome - that's beautiful.",
            practice:
              "Reframe failure as 'valuable learning data' rather than personal inadequacy.",
            reframe:
              "Every successful person has a history of failures that taught them what works.",
          };
          break;
        case "imposter_syndrome":
          guidance[block] = {
            insight:
              "Imposter syndrome often appears when you're growing into new levels of capability.",
            practice:
              "List evidence of your qualifications and past successes - you belong here.",
            reframe:
              "Feeling like an imposter means you're expanding beyond your comfort zone - that's growth.",
          };
          break;
      }
    });
  ...
  generateNextSteps(analysis) {
    ...
    steps.push(
      "Acknowledge yourself for doing this trust check-in - this itself is an act of self-care and growth",
    );
    ...
    if (trust_level >= 7 && readiness_level === "high") {
      ...
      steps.push(
        "Notice how it feels to operate from this level of self-trust - this is your natural state",
      );
    } else if (trust_level >= 5) {
      steps.push(
        'Practice the trust temperature check exercise: pause three times today and ask "How much do I trust myself right now?"',
      );
      steps.push(
        "Make one small decision purely from your gut feeling - like which route to take or what to eat",
      );
    } else {
      steps.push(
        "Practice micro-trust building: trust yourself with one tiny decision today",
      );
      steps.push(
    ...
    if (trust_blocks.includes("external_validation")) {
      steps.push(
        'Before asking others for advice today, first ask yourself: "What do I think about this?"',
      );
    }

    if (trust_blocks.includes("perfectionism")) {
      steps.push(
        'Take one imperfect action today - something that\'s "good enough" rather than perfect',
      );
    }
    ...
    if (readiness_level === "high") {
      steps.push(
        "Your high readiness creates perfect conditions for growth - choose one trust exercise to practice today",
      );
    } else if (readiness_level === "low") {
      steps.push(
        "Honor your low energy by being extra gentle with yourself - trust building happens in rest too",
      );
    }
  ...
  identifyCurrentStrengths(analysis) {
    ...
    if (analysis.readiness_level === "high") {
      strengths.push(
        "Your willingness and motivation for growth are powerful assets",
      );
    }
  ...
  generateConfidencePractices(analysis) {
    const practices = [
      {
        ...
        instruction:
          'Three times today, pause and ask: "How much do I trust myself right now?" Just notice, no judgment.',
    ...
    if (analysis.trust_level >= 6) {
      practices.push({
        ...
        instruction:
          "Identify one area where you could trust yourself 10% more and take one small action there.",
    ...
    if (analysis.body_awareness.length > 0) {
      practices.push({
        ...
        instruction:
          "Before decisions, place hand on belly and notice: does this option create expansion or contraction?",
  ...
  getInternalAuthorityPractices(analysis) {
    ...
    if (analysis.internal_vs_external === "external_focused") {
      practices.push(
        "Before seeking outside advice, spend 5 minutes journaling your own thoughts first",
      );
      practices.push(
        "Practice making small decisions without consulting others - like what to wear or eat",
      );
    }

    if (analysis.body_awareness.length === 0) {
      practices.push(
        "Develop body awareness: pause before decisions and notice physical sensations",
      );
    }

    practices.push(
      'End each day by asking: "What did I learn about trusting myself today?"',
    );
  ...
  async identifyTrustPatterns(analysis) {
    try {
      ...
      const patterns = {
        trust_level_pattern: this.analyzeTrustLevelPattern(
          recentSessions,
          analysis.trust_level,
        ),
        recurring_blocks: this.analyzeRecurringBlocks(
          recentSessions,
          analysis.trust_blocks,
        ),
        growth_trajectory: this.analyzeGrowthTrajectory(recentSessions),
        readiness_patterns: this.analyzeReadinessPattern(
          recentSessions,
      ...
      return {
        message:
          "Pattern analysis building - more check-ins will reveal deeper insights",
      };
    }
  }
  ...
  analyzeTrustLevelPattern(sessions, currentLevel) {
    if (sessions.length < 3) {
      return {
        pattern: "establishing_baseline",
        message: "Building your trust level baseline",
      };
    }
    ...
    if (currentLevel > average + 0.5) {
      return {
        pattern: "ascending",
        message: "Your trust in yourself is genuinely increasing",
      };
    } else if (currentLevel < average - 0.5) {
      return {
        pattern: "fluctuating",
        message: "Trust levels vary - this is normal and part of the process",
      };
    } else {
      return {
        pattern: "stabilizing",
        message: "Your trust level is stabilizing at a healthy baseline",
      };
    }
  }
  ...
  analyzeRecurringBlocks(sessions, currentBlocks) {
    ...
    if (recurringBlocks.length > 0) {
      return {
        ...
        growth_opportunity:
          "Recurring blocks show consistent growth edges - these are your development priorities",
      };
    }

    return {
      message:
        "No recurring blocks identified yet - keep tracking for patterns",
    };
  }
  ...
  analyzeGrowthTrajectory(sessions) {
    if (sessions.length < 5) {
      return {
        trajectory: "early_stage",
        message: "Building data for trajectory analysis",
      };
    }

    const recentAvg =
      sessions.slice(0, 3).reduce((sum, s) => sum + (s.trust_level || 5), 0) /
      3;
    const earlierAvg =
      sessions.slice(-3).reduce((sum, s) => sum + (s.trust_level || 5), 0) / 3;

    if (recentAvg > earlierAvg + 0.5) {
      return {
        trajectory: "strong_growth",
        message: "Clear upward trajectory in trust development",
      };
    } else if (recentAvg > earlierAvg) {
      return {
        trajectory: "steady_growth",
        message: "Consistent, steady progress in building self-trust",
      };
    } else {
      return {
        trajectory: "stabilizing",
        message: "Establishing a stable foundation of self-trust",
      };
    }
  }

  analyzeReadinessPattern(sessions, currentReadiness) {
    const readinessLevels = sessions.map(
      (s) => s.insights?.readiness_level || "moderate",
    );
    ...
    if (highReadiness >= 2 && currentReadiness === "high") {
      return {
        ...
        message:
          "You consistently show up ready for growth - this accelerates development",
      };
    } else if (currentReadiness === "high") {
      return {
        pattern: "ready_for_breakthrough",
        message:
          "Your high readiness creates perfect conditions for significant progress",
      };
    } else {
      return {
        pattern: "building_readiness",
        message: "Readiness fluctuates naturally - honor your current capacity",
      };
    }
  }

  connectToStandingTall(analysis) {
    const { trust_level, confidence_indicators, internal_vs_external } =
      analysis;
  ...
  identifyCelebrationMoments(analysis) {
    ...
    if (analysis.trust_level >= 7) {
      celebrations.push(
        "🎉 Your trust level is genuinely high - this is real progress worth celebrating!",
      );
    }
    ...
    if (analysis.confidence_indicators.length > 0) {
      celebrations.push(
        "💪 You demonstrated clear confidence indicators - you trust yourself more than you realize",
      );
    }

    if (analysis.internal_vs_external === "internal_focused") {
      celebrations.push(
        "🌟 You're primarily referencing your inner authority - this is sophisticated emotional intelligence",
      );
    }

    celebrations.push(
      "🌱 You chose to do this trust check-in - this decision itself shows self-care and growth commitment",
    );
  ...
  generateGrowthInsights(analysis) {
    ...
    if (analysis.situational_analysis) {
      insights.push(
        `This ${analysis.situational_analysis.situation_types[0]} situation is perfect for practicing trust in real-world application`,
      );
    }
    ...
    if (analysis.body_awareness.length > 0) {
      insights.push(
        "Your body awareness gives you direct access to wisdom - this is a powerful trust-building tool",
      );
    }

    insights.push(
      "Every trust check-in strengthens your internal authority and builds evidence of your capacity for self-guidance",
    );
  ...
  async createPersonalizedExercises(analysis) {
    ...
    if (analysis.trust_level <= 4) {
      exercises.push({
        name: "Micro-Trust Building",
        instruction:
          "Make one tiny decision today purely from your gut feeling - like which route to take or what to eat first.",
        purpose: "Practice trusting yourself in low-stakes situations",
        frequency: "Daily",
        difficulty: "Easy",
      });
      ...
      exercises.push({
        ...
        instruction:
          "Identify one area where you could trust yourself 10% more and take one small action there today.",
    ...
    if (analysis.trust_blocks.includes("external_validation")) {
      exercises.push({
        ...
        instruction:
          "Before seeking others' opinions today, spend 5 minutes writing your own thoughts first.",
        purpose:
          "Develop internal reference points before external consultation",
        frequency: "As needed",
        difficulty: "Medium",
      });
    }
    ...
    if (analysis.trust_blocks.includes("perfectionism")) {
      exercises.push({
        ...
        instruction:
          "Take one action today at 80% certainty rather than waiting for 100% - notice what happens.",
        purpose:
          "Build tolerance for imperfection and practice taking action despite uncertainty",
        frequency: "Daily",
        difficulty: "Medium",
      });
    }
    ...
    if (analysis.body_awareness.length > 0) {
      exercises.push({
        ...
        instruction:
          "Before your next decision, place hand on belly, present options, notice expansion (yes) or contraction (no).",
        purpose: "Use your body as a trust and decision-making tool",
  ...
  getEmergencyTrustResponse(data) {
    return {
      message:
        "Your willingness to check in with your trust levels shows beautiful self-awareness. The most important thing: you are already whole and worthy of your own trust.",
      trust_analysis: {
        trust_level: 5,
        message:
          "Your self-care in doing this check-in is evidence of your capacity for self-trust",
        emotional_state: ["curious", "caring"],
      },
      personalized_guidance: {
        main_message:
          "Trust building happens in small moments of choosing yourself over doubt.",
        next_steps: [
      ...
      trust_exercises: [
        {
          ...
          instruction:
            "End today by naming one way you trusted yourself, even if it felt small",
      ...
      standing_tall_connection:
        "Every moment you honor your inner experience, you practice standing tall in your truth.",
    };
  }
}
