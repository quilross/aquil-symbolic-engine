...
export class StandingTallCoach {
  ...
  async generatePractice(data) {
    try {
      ...
      const confidenceBuilding = await this.generateConfidenceBuilding(analysis);
  ...
  async analyzeStandingTallChallenge(data) {
    ...
    return {
      shrinking_patterns: this.identifyShrinkingPatterns(situation, fears_concerns),
  ...
  identifyShrinkingPatterns(situation, fears) {
    ...
    const shrinkingIndicators = {
      'people_pleasing': {
        keywords: ['please others', 'what they think', 'avoid conflict', 'keep everyone happy'],
        description: 'Prioritizing others\' comfort over your authenticity',
        transformation_opportunity: 'Learning that authenticity serves others more than people-pleasing'
      },
      'hiding': {
        keywords: ['hide', 'invisible', 'background', 'blend in', 'not seen'],
        description: 'Making yourself small to avoid judgment or responsibility',
        transformation_opportunity: 'Discovering that your visibility gives others permission to shine too'
      },
      'perfectionism': {
        keywords: ['perfect', 'right way', 'mistake', 'wrong', 'flawless'],
        description: 'Requiring perfection before feeling safe to act or be seen',
        transformation_opportunity: 'Embracing "good enough" and trusting your natural excellence'
      },
      'seeking_approval': {
        keywords: ['approval', 'validation', 'acceptance', 'liked', 'permission'],
        description: 'Looking outside yourself for validation and permission',
        transformation_opportunity: 'Developing internal validation and self-authority'
      },
      'minimizing': {
        keywords: ['not important', 'doesn\'t matter', 'small thing', 'no big deal'],
        description: 'Making your needs, feelings, or contributions seem smaller than they are',
        transformation_opportunity: 'Recognizing and claiming the full value of your presence and contributions'
      }
    };
    ...
    return patterns.length > 0 ? patterns : [{
      ...
      description: 'Uncertainty about how to show up authentically and powerfully',
      transformation_opportunity: 'Learning to trust your natural presence and authentic expression'
    }];
  }
  ...
  analyzeFears(fearsText) {
    ...
    const fearMap = {
      'judgment': {
        ...
        wisdom: 'Your fear of judgment shows you value authenticity and want to be seen truly',
        transformation: 'Learning that others\' judgments say more about them than about you',
        growth_edge: 'What would you do if others\' opinions had no power over you?'
      },
      'rejection': {
        keywords: ['reject', 'not liked', 'abandoned', 'excluded'],
        wisdom: 'Your fear of rejection shows you value belonging and connection',
        transformation: 'Discovering that the right people will appreciate your authenticity',
        growth_edge: 'What if rejection is redirection toward your true tribe?'
      },
      'failure': {
        keywords: ['fail', 'wrong', 'mistake', 'mess up'],
        wisdom: 'Your fear of failure shows you care about meaningful contribution',
        transformation: 'Reframing "failure" as valuable learning data and course correction',
        growth_edge: 'What would you attempt if failure was just information?'
      },
      'visibility': {
        keywords: ['seen', 'noticed', 'attention', 'spotlight', 'visible'],
        wisdom: 'Your fear of visibility often means you have important gifts that want to be shared',
        transformation: 'Understanding that your visibility gives others permission to shine',
        growth_edge: 'What wants to be expressed through you that the world needs to see?'
      },
      'inadequacy': {
        keywords: ['not enough', 'inadequate', 'not qualified', 'imposter'],
        wisdom: 'Your fear of inadequacy shows you have high standards and want to contribute meaningfully',
        transformation: 'Recognizing that everyone feels inadequate sometimes - it\'s part of growing',
        growth_edge: 'What if your "inadequacy" is actually humility preparing you for greater service?'
      }
    };
  ...
  extractExistingStrengths(pastSuccesses) {
    if (!pastSuccesses) return ['Your willingness to work on standing tall is already a significant strength'];
    ...
    Object.entries(strengthIndicators).forEach(([strength, keywords]) => {
      if (keywords.some(keyword => lowerSuccesses.includes(keyword))) {
        switch (strength) {
          case 'spoke_up':
            strengths.push('You have demonstrated courage in speaking your truth');
            break;
          case 'stood_ground':
            strengths.push('You have shown ability to stand firm in what matters to you');
            break;
          case 'took_action':
            strengths.push('You have the capacity to move from intention into action');
            break;
          case 'helped_others':
            strengths.push('You naturally support others - this shows your generous heart and leadership capacity');
            break;
        }
      }
    });
    ...
    if (strengths.length === 0) {
      strengths.push('You have succeeded before - these experiences are evidence of your capacity');
    }

    strengths.push('Your self-awareness about when you shrink shows sophisticated emotional intelligence');
  ...
  assessReadinessForGrowth(data) {
    ...
    const combinedText = `${situation} ${desired_outcome} ${fears_concerns || ''}`.toLowerCase();

    if (combinedText.includes('ready') || combinedText.includes('willing') || combinedText.includes('excited')) return 'high';
    if (combinedText.includes('scared') || combinedText.includes('overwhelming') || combinedText.includes('too much')) return 'gentle';
    return 'moderate';
  }
  ...
  clarifyStandingTallVision(desiredOutcome) {
    if (!desiredOutcome) {
      return {
        vision: 'Express yourself authentically and take up your rightful space in the world',
        embodied_version: 'You naturally standing in your power with confidence and presence',
        identity_shift: 'Becoming someone who naturally belongs wherever you are'
      };
    }
  ...
  identifyPowerLeaks(situation, fears) {
    ...
    if (combinedText.includes('quiet') || combinedText.includes('don\'t speak')) {
      leaks.push({
    ...
    if (leaks.length === 0) {
      leaks.push({
        ...
        description: 'General tendency to make yourself smaller than you naturally are',
  ...
  generatePracticePhases(analysis) {
    ...
    phases.push({
      ...
      duration: '3-4 minutes',
      purpose: 'Integrating the practice and setting intentions for standing tall',
      instructions: [
    ...
    analysis.shrinking_patterns.forEach(pattern => {
      switch (pattern.pattern) {
        case 'people_pleasing':
          beliefs.push('I can care deeply about others while honoring my own needs and truth');
          break;
        case 'perfectionism':
          beliefs.push('My authentic effort is more valuable than perfect performance');
          break;
        case 'seeking_approval':
          beliefs.push('My own approval of myself is the most important validation I can receive');
          break;
        default:
  ...
  generateConfidenceExperiments(analysis) {
    const experiments = [
      {
        ...
        instruction: 'Spend one hour maintaining confident posture - shoulders back, head high, taking up your natural space',
        difficulty: 'easy',
        purpose: 'Build physical confidence and notice how posture affects internal state'
      },
      {
        name: 'Authentic Opinion Sharing',
        instruction: 'Share your genuine opinion on one topic today, even if it differs from others',
        difficulty: 'medium',
        purpose: 'Practice trusting and expressing your authentic perspective'
      },
      {
        name: 'Space Claiming Practice',
        instruction: 'In your next social interaction, consciously take up your rightful physical and energetic space',
        difficulty: 'medium',
    ...
    if (analysis.shrinking_patterns.some(p => p.pattern === 'hiding')) {
      experiments.push({
        ...
        instruction: 'Make yourself slightly more visible than usual - speak up in a meeting, post something authentic, or wear something that expresses you',
  ...
  generatePosturePractices() {
    return [
      {
        name: 'Mountain Pose',
        instruction: 'Stand with feet rooted, spine tall, crown reaching skyward - breathe into your natural majesty',
      ...
      },
      {
        ...
        instruction: 'Walk with shoulders back, chest open, head high - own the ground beneath your feet',
        when: 'Throughout the day, especially when entering new spaces',
        purpose: 'Practice moving through the world with presence and confidence'
      },
      {
        name: 'Seated Presence',
        instruction: 'Sit with spine erect, shoulders relaxed but broad, taking up your natural space in the chair',
  ...
  generateEnergyReclaimingPractices(analysis) {
    ...
    practices.push({
      ...
      practice: 'Throughout the day, notice when you\'re making yourself smaller and consciously expand back to your natural size',
  ...
  getFearSpecificPractice(fear) {
    const practices = {
      'judgment': 'Practice: Ask yourself "Whose opinion actually matters to me and why?" Focus on the voices that truly support your growth',
      'rejection': 'Practice: Remind yourself "The right people will appreciate my authenticity" before sharing something true',
      'failure': 'Practice: Reframe each "failure" as valuable data - ask "What did this teach me about what works?"',
      'visibility': 'Practice: Start with small acts of authentic visibility and notice that the sky doesn\'t fall',
      'inadequacy': 'Practice: When feeling inadequate, ask "What if this feeling is preparing me to serve more effectively?"'
    };
    return practices[fear] || 'Practice: Breathe through the fear and take one small brave action aligned with your truth';
  }
    ...
    return [
      {
        ...
        instruction: 'Spend 2 minutes in confident posture while setting your intention to stand tall today',
        timing: 'Morning',
        purpose: 'Start each day grounded in your natural power'
      },
      {
        name: 'Courage Micro-Moments',
        instruction: 'Take one small action today that requires you to be slightly more visible or authentic',
        timing: 'During the day',
        purpose: 'Build courage through consistent small steps'
      },
      {
        name: 'Evening Acknowledgment',
        instruction: 'Before bed, acknowledge one moment when you stood tall today (even if it felt small)',
        timing: 'Evening',
        purpose: 'Build evidence of your growing confidence'
      },
      {
        name: 'Shrinking Pattern Interrupt',
        instruction: 'When you notice yourself shrinking, pause, breathe, and consciously expand back to your natural size',
        timing: 'As needed',
        purpose: 'Develop real-time awareness and choice around shrinking patterns'
      }
    ];
  }
  ...
  integrateWithTrustBuilding(analysis) {
    return {
      trust_connection: 'Standing tall is the external expression of internal self-trust - they develop together',
      trust_practices: [
      ...
      integration_insight: 'You can\'t authentically stand tall in the world if you don\'t trust your inner authority - they\'re two sides of the same developmental process'
    };
  }
  ...
  generateWelcomeMessage(analysis) {
    if (analysis.readiness_for_growth === 'high' && analysis.confidence_level >= 6) {
  ...
  getEmergencyStandingTallResponse(data) {
    return {
      message: "Your desire to stand tall instead of shrinking is a powerful choice. Remember: you don\'t need to earn the right to take up space - it\'s already yours.",
      situation_analysis: { 
        message: "Working on standing tall shows you\'re choosing expansion over contraction - this is courage in action",
        confidence_level: 5
      },
      ...
      daily_empowerment_practices: [{
        ...
        instruction: "Each morning, stand tall and remind yourself: 'I belong here and my presence matters'",
