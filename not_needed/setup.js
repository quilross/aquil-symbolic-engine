...
async function setup() {
  try {
    ...
    const dbResult = execSync("wrangler d1 create aquil-wisdom-db", {
      encoding: "utf8",
    });
    ...
    if (!dbId) {
      ...
      console.log(
        "Manual step: Copy the database_id from above and update wrangler.toml",
      );
      return;
    }
    ...
    const kvResult = execSync('wrangler kv:namespace create "AQUIL_MEMORIES"', {
      encoding: "utf8",
    });
    ...
    const kvPreviewResult = execSync(
    ...
    if (dbId && kvId && kvPreviewId) {
      ...
      wranglerConfig = wranglerConfig.replace(
        "REPLACE_WITH_ACTUAL_DATABASE_ID",
        dbId,
      );
      wranglerConfig = wranglerConfig.replace(
        "REPLACE_WITH_ACTUAL_KV_ID",
        kvId,
      );
      wranglerConfig = wranglerConfig.replace(
        "REPLACE_WITH_ACTUAL_KV_PREVIEW_ID",
        kvPreviewId,
      );
      wranglerConfig = wranglerConfig.replace(
        "REPLACE_WITH_DEV_DATABASE_ID",
        dbId,
      );
      ...
      console.log(
        "Please manually update wrangler.toml with the IDs shown above.",
      );
      console.log("\nResource IDs found:");
    ...
    console.log(
      "5. wrangler d1 execute AQUIL_DB --file=schema.sql --env production",
    );
  }
}
