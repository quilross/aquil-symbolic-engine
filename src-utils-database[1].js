...
export class AquilDatabase {
  ...
  async getUserProfile() {
    try {
      const result = await this.db.prepare('SELECT * FROM user_profile WHERE id = ?')
        .bind('aquil_user')
        .first();
  ...
  async getRecentSomaticSessions(limit = 10) {
    try {
      ...
      return results.results?.map(row => ({
        ...
        trust_building_elements: JSON.parse(row.trust_building_elements || '{}'),
  ...
  async storeMemory(key, data, ttl = 86400) { // 24 hours default
    try {
