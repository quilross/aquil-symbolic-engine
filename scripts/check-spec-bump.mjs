 ...
 * Warns when gpt-actions-schema.json changes without version bump
 */
...
async function checkSpecBump() {
  try {
    ...
    // Check if gpt-actions-schema.json was modified in the last commit
    let changedFiles;
    try {
    ...
    const schemaChanged = changedFiles.includes('gpt-actions-schema.json');
    ...
    const schemaPath = join(rootDir, 'gpt-actions-schema.json');
    ...
    let previousVersion;
    try {
      const previousSchema = execSync('git show HEAD~1:gpt-actions-schema.json', {
        encoding: 'utf8',
        cwd: rootDir
      });
